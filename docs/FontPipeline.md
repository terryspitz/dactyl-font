...
- **Sharp, inner**: there is no single point that belongs to both offset edges (a bisector miter lands off both of them, worse the sharper the corner — this used to taper `5`'s stem into its acute bowl join). So no join point is invented: the incoming edge ends at its own true endpoint, the outgoing edge starts at its own true endpoint, and the two bodies are left to overlap — the nonzero fill rule unions them into the ink a stroke of that width actually lays down.
