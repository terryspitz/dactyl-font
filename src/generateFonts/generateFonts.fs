...
let main argv =
    ...
    let fonts =
        [
            ...
            ("Dactyl Sans Extra Light", Font({ Axes.DefaultAxes with weight = 3 }))
            ("Dactyl Sans", Font({ Axes.DefaultAxes with weight = 30 }))
            ("Dactyl Sans Italic", Font({ Axes.DefaultAxes with slant = 0.15 }))
            ("Dactyl Sans Bold", Font({ Axes.DefaultAxes with weight = 60 }))
            ("Dactyl Round",
            Font(
                { Axes.DefaultAxes with
                    end_bulb = 0.5
    ...
    if outputProofs then
        ...
        let splitWords (s: string) =
            s.Split('\n', '\r')
            |> Seq.filter (fun w -> w.Length > 0 && w.[0] <> '#')
        ...
        for i in 0 .. fonts.Length - 1 do
            ...
            let wrap lines (w: string) =
                ...
                match lines with
                | head :: tail ->
                    if (w.Length + lineLength head) < charsPerLine then
                        (head @ [ w ]) :: tail
                    else
                        [ w ] :: head :: tail
                | [] -> [ [ w ] ]
            ...
            let lines =
                name
                :: (List.fold wrap [] lowercase |> List.map (String.concat " ") |> List.rev)
