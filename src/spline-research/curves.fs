...
type Spline2 (ctrlPts, isClosed) =
    ...
    member private this.renderTangentsFiltered (includeKnot: Spline2ControlPoint -> bool) =
        let path = BezPath()
        let length = this.ctrlPts.Length - if this.isClosed then 0 else 1
        for i in 0..length do
            if i < this.ctrlPts.Length then
                let cp = ctrlPts.[i]
                if includeKnot cp then
                    let ptI = this.pt(i, 0)
                    let offset = 100.
    ...
    member this.renderTangents = this.renderTangentsFiltered (fun _ -> true)
    ...
    member this.renderExplicitTangents =
        this.renderTangentsFiltered (fun cp -> cp.lth.IsSome || cp.rth.IsSome)
