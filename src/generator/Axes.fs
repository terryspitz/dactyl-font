...
type Axes =
    { dactyl_spline: bool //use new dactyl splines with new glyph definitions
      ...
      pressure: float //stroke thickens at curves, thins on straights, simulating brush pressure (0=off, 1=max)
    ...
    static member controls =
        [ "dactyl_spline", Checkbox, "experimental"
          ...
          "pressure", FracRange(0.0, 1.0), "artistic"
    ...
    member this.sampledArtistic =
        this.nib > 0.0 || this.taper > 0.0 || this.wobble > 0.0 || this.pressure > 0.0 || this.mobius > 0.0
