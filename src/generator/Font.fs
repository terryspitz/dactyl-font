...
type Font(axes: Axes, ?showCombOpt: bool) =
    ...
    member this.getOutline =
        if this.axes.stroked then
            ...
            // must use the Spiro outline path — constant_offset is a DactylSpline-only generation
            // method and is ignored in that case.  Only when dactyl_spline is on does
            // constant_offset choose between the two DactylSpline outline strategies.
            (if not axes.dactyl_spline then
                 this.getSpiroSansOutlines
             elif axes.constant_offset then
