...
/// Bezier context that captures both the SVG path data and a curvature comb for Spiro curves.
/// Pass showComb=true to compute comb spurs; false yields just the path with empty comb.
...
type Font(axes: Axes) =
    ...
    member this.spiroToSvgWithComb spiro =
        ...
        | SpiroOpenCurve(segs) ->
            ...
            Spiro.SpirosToBezier (Array.ofList segs) false (bc :> IBezierContext) |> ignore
            ([ bc.GetPathData ], bc.GetCombSvg)
        | SpiroClosedCurve(segs) ->
            let bc = SpiroCombContext(axes.show_comb)
            Spiro.SpirosToBezier (Array.ofList segs) true (bc :> IBezierContext) |> ignore
            ([ bc.GetPathData ], bc.GetCombSvg)
