...
type GlyphFsDefs(axes: Axes) =
    ...
    member this.getGlyph e =
        ...
        | Glyph('#') ->
            ...
            EList(
                [ Line(YX(Y3 * 2, L), YX(Y3 * 2, R))
                  Line(YX(Y3, L), YX(Y3, R))
                  Line(YX(T, R - R / 4), YX(B, R - R / 4))
                  Line(YX(T, R / 4), YX(B, R / 4)) ]
            )
        | Glyph('£') ->
            EList(
                [ openCurve
        ...
        | Glyph('&') ->
            openCurve
                [ (BR, Start)
                  ...
                  (TC, G2)
                  (YX(T - flooredOffset, R - offset), G2)
                  (YX(H, (R - offset) / 2), G2)
        ...
        | Glyph('*') ->
            ...
            EList(
                [ Line(YX(T * 2 / 3, L), YX(T * 2 / 3, N))
                  Line(YX(T * 2 / 3 + sin30, R / 5), YX(T * 2 / 3 - sin30, N * 4 / 5))
                  Line(YX(T * 2 / 3 + sin30, N * 4 / 5), YX(T * 2 / 3 - sin30, R / 5)) ]
            )
        ...
        | Glyph('@') ->
            openCurve
                [ (YX(T / 3, R * 3 / 4), Corner)
                  ...
                  (YX(H, R / 4), G2)
                  (YX(T * 2 / 3, C), G2)
                  (YX(H, R * 3 / 4), CurveToLine)
                  (YX(T / 3, R * 3 / 4), LineToCurve)
                  (YX(T / 4, R), CurveToLine)
                  (YX(T / 2, R), LineToCurve)
                  (TC, G2)
                  (HL, G2)
                  (BLo, G2)
                  (BR, End) ]
        ...
        | Glyph('1') ->
            ...
            EList(
                [ PolyLine([ YX(T * 4 / 5, L); YX(T, midX); YX(B, midX) ]) ]
        ...
        | Glyph('8') ->
            closedCurve
                [ (TC, G2)
                  (Mid(TL, HL), G2)
                  (HC, Anchor)
                  (YX(T * 4 / 10, C + flooredOffsetHalf), Handle)
                  (Mid(HR, BR), G2)
                  (BC, G2)
                  (Mid(HL, BL), G2)
                  (HC, Anchor)
        ...
        | Glyph('i') ->
            ...
            EList(
                [ Line(YX(X, midX), YX(B, midX)); Dot(YX(dotHeight, midX)) ]
                @ if axes.monospace > 0.0 then
                      [ Line(BL, YX(B, midX * 2)); Line(XL, YX(X, midX)) ]
                  else
                      []
            )
        ...
        | Glyph('S') ->
            openCurve
                [ (ToR, G2)
                  (TC, G2)
                  (Mid(TL, HL), G2)
                  (YX(H * 11 / 10, C - offset), G2)
                  (YX(H * 9 / 10, C + offset), G2)
                  (Mid(HR, BR), G2)
                  (BC, G2)
                  (BoL, End) ]
        | Glyph('s') ->
        ...
        //default
        | Glyph(c) ->
            printfn "Glyph %c not defined" c
            Dot(XC)
        | any -> any

    member this.reduce e =
        match e with
        | Line(p1, p2) -> Curve([ { pt = YX(reducePoint p1); ty = Start; th_in = None; th_out = None }; { pt = YX(reducePoint p2); ty = End; th_in = None; th_out = None } ], false)
        | PolyLine(points) ->
            let a = Array.ofList points

            Curve(
                [ for i in 0 .. a.Length - 1 do
                      yield { pt = YX(reducePoint a.[i]); ty = (if i = (a.Length - 1) then End else Corner); th_in = None; th_out = None } ],
                false
            )
        | Curve(pts, isClosed) ->
            Curve(
                [ for k in pts do
                      { k with pt = YX(reducePoint k.pt) } ],
                isClosed
            )
