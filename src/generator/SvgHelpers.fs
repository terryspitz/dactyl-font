...
let svgCircle (x: float) (y: float) (r: float) =
    [ sprintf "M %.0f,%.0f" (x - r) y
      sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x - r) (y + r / 2.0) (x - r / 2.0) (y + r) x (y + r)
      sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x + r / 2.0) (y + r) (x + r) (y + r / 2.0) (x + r) y
      sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x + r) (y - r / 2.0) (x + r / 2.0) (y - r) x (y - r)
      sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x - r / 2.0) (y - r) (x - r) (y - r / 2.0) (x - r) y
      "Z" ]
...
let svgDiamond (x: float) (y: float) (r: float) =
    [ sprintf "M %.0f,%.0f" (x - r) y
      sprintf "L %.0f,%.0f" x (y + r)
      sprintf "L %.0f,%.0f" (x + r) y
      sprintf "L %.0f,%.0f" x (y - r)
      sprintf "L %.0f,%.0f" (x - r) y
      "Z" ]
...
let svgSemiCircle (x: float) (y: float) (r: float) (ch: char) =
    ...
    [ if ch = 'u' || ch = 'd' then
          ...
          sprintf "M %.0f,%.0f" x (y - r)
      if ch = 'u' then
          sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x - r) (y + r) (x + r) (y + r) (x + r) y
      elif ch = 'd' then
          sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x - r) (y - r) (x + r) (y - r) (x + r) y
      elif ch = 'l' then
          sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x - r) (y - r) (x - r) (y + r) x (y + r)
      elif ch = 'r' then
          sprintf "C %.0f,%.0f %.0f,%.0f %.0f,%.0f" (x + r) (y - r) (x + r) (y + r) x (y + r)
      "Z" ]

let svgText (x: float) (y: float) text =
    sprintf "<text x='%.0f' y='%.0f' font-size='200'>%s</text>" x y text

let toSvgDocument (left: float) (bottom: float) (width: float) (height: float) svg =
    [ "<svg xmlns='http://www.w3.org/2000/svg'"
      sprintf "viewBox='%.0f %.0f %.0f %.0f'>" left bottom width height
      "<g id='1'>" ]
    @ svg
    @ [ "</g>"; "</svg>" ]
        ...
        | Curve(knots, _) ->
            ...
            |> List.choose (fun k ->
                ...
                |> Option.map (fun lbl ->
                    ...
                    sprintf "<text x='%.0f' y='%.0f' font-family='monospace' font-size='40' fill='red' font-weight='bold'>%s</text>" (x + 15.0) (y - 15.0) lbl))
