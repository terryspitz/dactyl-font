...
module glyphs =
    ...
    type Font (axes: Axes) =
        ...
        member this.getGlyph e =
            ...
            // :;<=>?@
            // [\]^_` {|}~
            | Glyph("adgqLoop") -> ClosedCurve([(XoR, Corner); (XC, G2); (ML, G2); (BC, G2); (BoR, Corner)])
            | Glyph("A") -> let f = float(H/2)/float(T)
                            List([PolyLine([BL; TC; BR]); PolyLine([BL; Interp(BL,TC,f); Interp(BR,TC,f); BR])])
            | Glyph("a") -> List([Line(XR, BR); Glyph("adgqLoop")])
            ...
            | Glyph("d") -> List([Line(BR, TR); Glyph("adgqLoop")])
            ...
            | Glyph("g") -> List([Glyph("adgqLoop"); OpenCurve([(XR, Corner); (BR, LineToCurve); (DC, G2); (DoL, End)])])
            ...
            | Glyph("q") -> List([Line(XR, DR); Glyph("adgqLoop")])
            ...
            //default
            | Glyph(c) -> printfn "Glyph %s not defined" c
                          Dot(XC)
            | any -> any
        ...
        member this.stringToSvg (str : string) offsetX offsetY outlines filled points =
            let widths = [for ch in str do this.width (Glyph(string ch))]
            let offsetXs = List.scan (fun a e -> a+e+50) offsetX widths
            String.concat "\n"
                [for c in 0 .. str.Length - 1 do
                    printfn "%c" str.[c]
                    yield this.charToSvg (string str.[c]) (offsetXs.[c]) offsetY outlines filled points]
        ...
        member this.toFontForgeGlyph (ch : char) =
            ...
            let frameSpiros = Glyph(string ch) |> this.elementToSpiros |> 
                              List.map toFFSpiro |> concatLines
            // let frameSpiros = Font({this.axes with thickness = 2}).getOutlines (Glyph(string ch)) 
            //                   |> this.elementToSpiros |> List.map toString |> concatLines
            let outlineSpiros = this.getOutlines (Glyph(string ch)) |> this.elementToSpiros |> 
                                List.map toFFSpiro |> concatLines
            sprintf "StartChar: %c\n" ch +
            sprintf "Encoding: %d %d 0\n" (int ch) (int ch) +
            sprintf "Width: %d\n" (this.width(Glyph(string ch)) + thickness) +
            sprintf """
                    InSpiro: 1
                    Flags: H
                    LayerCount: 2
...
let main argv =
    ...
    let outputInterpolatedStr = true
