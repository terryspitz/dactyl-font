...
module glyphs =
    ...
    let rec rewritePoint (axes : Axes) p = 
        ...
        | Mid(p1, p2) -> rewritePoint axes (Interp(p1, p2, 0.5))
        | Interp(p1, p2, f) -> let x1, y1 = (getXY axes p1) in let x2, y2 = (getXY axes p2)
                               YX(y1+int(float(y2-y1)*f), x1+int(float(x2-x1)*f))
    and getXY axes p =
        match (rewritePoint axes p) with 
            YX(y,x) -> (x, y)
    ...
    and reduce axes e =
        match e with
        | Line(p1, p2) -> OpenCurve([(rewritePoint axes p1, Start); (rewritePoint axes p2, End)])
        | PolyLine(points) -> let a = Array.ofList points
                              OpenCurve([for i in 0 .. a.Length-1 do
                                         let p = rewritePoint axes a.[i]
                                         yield (p, if i=(a.Length-1) then End else Corner)])
        | OpenCurve(curvePoints) -> OpenCurve([for p, t in curvePoints do rewritePoint axes p, t])
        | ClosedCurve(curvePoints) -> ClosedCurve([for p, t in curvePoints do rewritePoint axes p, t])
        | Dot(p) -> Dot(rewritePoint axes p)
        | List(el) -> List(List.map ((getGlyph axes) >> (reduce axes))  el)
        | e -> reduce axes (getGlyph axes e)
    ...
    let rec getWidth axes el =
        match reduce axes el with
        | OpenCurve(curvePoints) -> List.fold max 0 (List.map (fst >> (getXY axes) >> fst) curvePoints)
        | ClosedCurve(curvePoints) -> List.fold max 0 (List.map (fst >> (getXY axes) >> fst) curvePoints)
        | Dot(p) -> fst(getXY axes p)
        | List(el) -> List.fold max 0 (List.map (getWidth axes) el)

    let rec elementToSpiros axes e =
        match reduce axes e with
    ...
    let getOutlines e thickness = 
        ...
        let offsetPointCap(X, Y, theta) = offsetPointByThickness(X, Y, theta, thickness * sqrt 2.0 * holdoff)
    ...
    let toSvgBezierCurve axes spiro = 
        match spiro with
        | SpiroOpenCurve(scps) ->
        ...
        | SpiroClosedCurve(scps) ->
            ...
            bc.ToString()
        | SpiroDot(p) -> let x, y = getXY axes p
                         svgCircle x y axes.thickness

    let getSvgCurves axes element offsetX offsetY strokeWidth filled outlines thickness =
    ...
    let getSvgPoints axes element offsetX offsetY =
        let toSvgPoints (spiro : SpiroElement) : string = 
            ...
            | SpiroDot(p) -> let x,y = (getXY axes p) in point(float(x), float(y))
    ...
    let charToSvg axes ch offsetX offsetY outlines filled points =
        let element = Glyph(ch)
        let path = getSvgCurves axes element offsetX offsetY 20 false false
        sprintf "<!-- %c -->\n\n" ch +
        if outlines then
            getSvgCurves axes element offsetX offsetY 5 filled outlines +
                (if filled then "" else path) +
                (if points then getSvgPoints axes element offsetX offsetY else "")
        else path + 
             (if points then getSvgPoints axes element offsetX offsetY else "")

    let toFontForgeGlyph axes (ch : char) thickness =
        ...
        let toString spiro =
            ...
            sprintf """
                    ...
                    EndSpiro
                    """ s
        let spiros = getOutlines axes (Glyph(ch)) (float thickness) |> elementToSpiros |> 
...
let main argv =
    ...
    let filled = true
    let points = true
    let axes = { width = 300; height = 600; x_height = 400; offset = 50; thickness = 10; }
    let rowHeight = 1024
    let thickness = 10
    ...
    let svg = [
        for r in 0 .. rows do
            ...
            let widths = [for ch in rowChars do glyphs.getWidth axes (glyphs.Glyph(ch))]
