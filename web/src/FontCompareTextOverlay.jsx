 ...
 */
export default function FontCompareTextOverlay({ text, fontFamily, dactylFamily, labelB }) {
  const chars = [...(text || '').replace(/[\n\r]/g, '')]
  const size = 90

  const cell = (ch) => {
    const glyph = (family, color, extra) => (
      <span style={{ fontFamily: family, fontSize: size, lineHeight: 1, color, ...extra }}>{ch}</span>
    )
    return (
      <div key={ch} className="compare-text-cell">
        <div className="compare-text-col">{glyph(dactylFamily, '#000')}</div>
        <div className="compare-text-col">{glyph(fontFamily, '#000')}</div>
        <div className="compare-text-col compare-text-overlay">
          {glyph(dactylFamily, 'rgba(255,0,0,0.55)', { position: 'absolute' })}
          {glyph(fontFamily, 'rgba(0,0,255,0.55)', { position: 'absolute' })}
        </div>
      </div>
    )
  }
